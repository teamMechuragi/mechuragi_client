...
import { getPreference, updatePreference, deletePreference } from "@/app/api/preferenceApi";
...
export default function EditDetailsClient({ id }: { id: string }) {
  ...
  const fetchPreferenceDetail = async () => {
    try {
      ...
      // API를 통한 선호도 조회
      const data = await getPreference(Number(preferenceId));
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      // API를 통한 선호도 수정
      await updatePreference(Number(preferenceId), updateData);
  ...
  const handleDelete = async () => {
    ...
    try {
      // API를 통한 선호도 삭제
      await deletePreference(Number(preferenceId));
